...
## ゲーム概要
極短時間で表示される複数の選択肢から正しいものを瞬時に判断・選択する瞬間的判断力と決断力のゲーム
...
<div class="instant-judgment-game">
  ...
  <div class="judgment-analytics" id="judgmentAnalytics">
    <div class="analytics-title">判断分析</div>
    <div class="analytics-charts">
      <canvas id="accuracyChart" width="300" height="80"></canvas>
      <canvas id="speedChart" width="300" height="80"></canvas>
    </div>
    <div class="type-performance" id="typePerformance">
      <div class="performance-item">
        <span class="type-label">正誤判断</span>
        <span class="type-accuracy" id="correctAccuracy">0%</span>
      </div>
      <div class="performance-item">
        <span class="type-label">大小比較</span>
        <span class="type-accuracy" id="comparisonAccuracy">0%</span>
      </div>
      <div class="performance-item">
        <span class="type-label">色彩識別</span>
        <span class="type-accuracy" id="colorAccuracy">0%</span>
      </div>
      <div class="performance-item">
        <span class="type-label">形状分類</span>
        <span class="type-accuracy" id="shapeAccuracy">0%</span>
      </div>
    </div>
  </div>
...
class InstantJudgmentGame {
  constructor() {
    ...
    this.typePerformance = {
      ...
      color: { total: 0, correct: 0 },
      shape: { total: 0, correct: 0 }
    };
  ...
  initializeElements() {
    this.elements = {
      ...
      correctAccuracy: document.getElementById('correctAccuracy'),
      comparisonAccuracy: document.getElementById('comparisonAccuracy'),
      colorAccuracy: document.getElementById('colorAccuracy'),
      shapeAccuracy: document.getElementById('shapeAccuracy')
    };
  }
  ...
  initializeCharts() {
    this.accuracyChart = this.elements.accuracyChart.getContext('2d');
    this.speedChart = this.elements.speedChart.getContext('2d');
    this.drawCharts();
  }
  ...
  startGame() {
    ...
    Object.keys(this.typePerformance).forEach(type => {
      this.typePerformance[type] = { total: 0, correct: 0 };
    });
  ...
  generateQuestion() {
    ...
    this.typePerformance[this.currentType].total++;
  ...
  selectChoice(choice) {
    ...
    if (isCorrect) {
      this.correctAnswers++;
      this.typePerformance[this.currentType].correct++;
    }
  ...
  updateTypePerformance() {
    Object.keys(this.typePerformance).forEach(type => {
      const performance = this.typePerformance[type];
      const accuracy = performance.total > 0 ? (performance.correct / performance.total * 100) : 0;
      
      switch (type) {
        case 'correct':
          this.elements.correctAccuracy.textContent = accuracy.toFixed(0) + '%';
          break;
        case 'comparison':
          this.elements.comparisonAccuracy.textContent = accuracy.toFixed(0) + '%';
          break;
        case 'color':
          this.elements.colorAccuracy.textContent = accuracy.toFixed(0) + '%';
          break;
        case 'shape':
          this.elements.shapeAccuracy.textContent = accuracy.toFixed(0) + '%';
          break;
      }
    });
  }
  ...
  drawAccuracyChart() {
    ...
    if (this.responses.length > 1) {
      ...
      for (let i = 0; i < this.responses.length; i++) {
        ...
        const x = (width / this.totalQuestions) * (i + 1);
    ...
    ctx.fillStyle = 'white';
    ctx.font = '12px Arial';
    ctx.fillText('正解率', 5, 15);
    ctx.fillText('100%', 5, 25);
    ctx.fillText('0%', 5, height - 5);
  }
  
  drawSpeedChart() {
    const ctx = this.speedChart;
    const width = ctx.canvas.width;
    const height = ctx.canvas.height;
    ...
    if (this.responses.length > 1) {
      ...
      const maxTime = 1000;
      for (let i = 0; i < this.responses.length; i++) {
        const time = Math.min(this.responses[i].time, maxTime);
        const x = (width / this.totalQuestions) * (i + 1);
        const y = height - (height / maxTime) * time;
...
## 技術仕様
- **フラッシュ表示**: 0.3秒間の極短時間表示
- **判断タイプ**: 正誤、大小、色彩、形状の4種類
- **制限時間**: 各問題1秒以内の回答必須
- **パフォーマンス測定**: タイプ別正解率追跡
- **リアルタイム分析**: 正解率・反応時間グラフ
- **キーボード対応**: A/B/C/Dキーでの回答可能
